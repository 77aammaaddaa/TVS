 ...
 * الوظيفة: إدارة المؤسسات، توليد أكواد التفعيل، مراقبة النظام الشاملة.
 */
...
const SuperAdminModule = ({ currentUser }) => {
    ...
    const loadSystemData = async () => {
        try {
            const orgs = await window.db.getAll('organizations').catch(() => []);
            const lics = await window.db.getAll('licenses').catch(() => []);
            setOrganizations(orgs);
            setLicenses(lics);
        } catch (error) {
            console.error("SuperAdmin DB Error:", error);
            // بيانات وهمية للعرض إذا لم تكن الجداول جاهزة بعد
            setOrganizations([
                { id: '1', name: 'سنتر عبد الله', owner: 'عبد الله', status: 'active', usersCount: 5, branchesCount: 2 },
                { id: '2', name: 'محلات التوحيد', owner: 'أحمد', status: 'suspended', usersCount: 1, branchesCount: 1 }
            ]);
            setLicenses([
                { id: '1', key: 'ECO-X12-ABCD-9876', org_id: '1', plan: 'platinum', status: 'active', expires_at: '2027-01-01' }
            ]);
        }
    };
    ...
    const handleCreateOrganizationAndLicense = async () => {
        if (!newLicenseData.orgName || !newLicenseData.ownerName) {
            return showNotification('error', '⚠️ يرجى إدخال اسم المؤسسة واسم المالك.');
        }

        const newOrgId = 'ORG_' + Date.now().toString(36).toUpperCase();
        ...
        const orgObj = {
            ...
            owner: newLicenseData.ownerName,
        ...
        const licenseObj = {
            id: 'LIC_' + Date.now().toString(36).toUpperCase(),
            key: newLicenseKey,
            org_id: newOrgId,
        ...
        try {
            await window.db.add('organizations', orgObj);
            await window.db.add('licenses', licenseObj);
            showNotification('success', `✅ تم إنشاء المؤسسة وتوليد الكود: ${newLicenseKey}`);
            
            // تفريغ وتحديث
            setNewLicenseData({ orgName: '', ownerName: '', phone: '', plan: 'gold', durationMonths: 12 });
            loadSystemData();
        } catch (error) {
            showNotification('error', '❌ فشل في حفظ بيانات التفعيل في قاعدة البيانات.');
        }
    };
    ...
    const toggleOrgStatus = async (orgId, currentStatus) => {
        ...
        try {
            await window.db.update('organizations', orgId, { status: newStatus });
        ...
        <div className="h-full flex flex-col md:flex-row bg-slate-900 text-slate-200 font-sans animate-in fade-in">
            ...
            <div className="w-full md:w-64 bg-slate-950 border-b md:border-b-0 md:border-l border-slate-800 p-6 shrink-0 flex flex-col">
                <div className="mb-8 border-b border-slate-800 pb-6 text-center">
                    ...
                    <h2 className="font-black text-xl text-white tracking-widest">ECOFINE <span className="text-indigo-400">CORE</span></h2>
                ...
                <div className="space-y-2 flex-1">
                    {[
                        ...
                        { id: 'organizations', icon: '🏢', label: 'المؤسسات والفروع' },
                ...
                <div className="mt-auto pt-6 border-t border-slate-800 text-center">
                    ...
                    <p className="text-[9px] text-slate-600 mt-1">V 12.0 Platinum Plus</p>
                </div>
            </div>
                ...
                {activeTab === 'dashboard' && (
                    <div className="space-y-8 animate-in slide-in-from-bottom-4">
                        <div>
                            ...
                            <p className="text-sm text-slate-400">نظرة عامة على إمبراطورية EcoFine</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="bg-slate-800 p-6 rounded-[2rem] border border-slate-700 shadow-xl relative overflow-hidden">
                                ...
                                <h4 className="text-xs text-slate-400 font-black uppercase tracking-widest mb-2 relative z-10">التراخيص النشطة</h4>
                                <span className="text-4xl font-black text-emerald-400 relative z-10">{licenses.filter(l=>l.status === 'active').length}</span>
                            </div>
                            <div className="bg-slate-800 p-6 rounded-[2rem] border border-slate-700 shadow-xl relative overflow-hidden">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-purple-500/10 rounded-full blur-3xl"></div>
                                <h4 className="text-xs text-slate-400 font-black uppercase tracking-widest mb-2 relative z-10">العملاء على الشبكة</h4>
                                <span className="text-4xl font-black text-purple-400 relative z-10">0 <span className="text-sm">قريباً</span></span>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'organizations' && (
                    <div className="space-y-6 animate-in slide-in-from-bottom-4">
                        <div className="flex justify-between items-center bg-slate-800 p-6 rounded-[2rem] border border-slate-700">
                            <div>
                                <h3 className="text-xl font-black text-white">المؤسسات المشتركة</h3>
                            ...
                            <table className="w-full text-right">
                                <thead className="bg-slate-950/50 text-[10px] uppercase tracking-widest text-slate-400">
                                    <tr>
                                        ...
                                        <th className="p-4 text-center">الفروع / المستخدمين</th>
                                    ...
                                    {organizations.map(org => (
                                        <tr key={org.id} className="hover:bg-slate-700/20 transition-colors">
                                            <td className="p-4">
                                                ...
                                                <span className="text-[10px] text-slate-500">{org.owner}</span>
                                            </td>
                                            <td className="p-4 text-slate-300">{org.phone || '---'}</td>
                                            <td className="p-4 text-center">
                                                <span className="bg-slate-900 px-3 py-1 rounded-lg text-xs border border-slate-700">{org.branchesCount || 0} فرع | {org.usersCount || 0} موظف</span>
                                            </td>
                                            <td className="p-4 text-center">
                                                <span className={`px-3 py-1 rounded-full text-[10px] uppercase tracking-widest ${org.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'}`}>
                                            ...
                                            <td className="p-4 text-center">
                                                <button 
                                                    ...
                                                    className={`px-4 py-2 rounded-xl text-xs transition-colors ${org.status === 'active' ? 'bg-rose-600/20 text-rose-400 hover:bg-rose-600 hover:text-white' : 'bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600 hover:text-white'}`}
                                                >
                                                    {org.status === 'active' ? 'إيقاف النظام' : 'تفعيل النظام'}
                                                </button>
                                            </td>
                                        </tr>
                                    ))}
                    ...
                    <div className="space-y-6 animate-in slide-in-from-bottom-4">
                        <div className="bg-gradient-to-br from-indigo-900 to-slate-900 p-8 rounded-[2rem] border border-indigo-500/30 shadow-2xl relative overflow-hidden">
                            ...
                            <h3 className="text-2xl font-black text-white mb-2 relative z-10 flex items-center gap-3">
                                <span className="text-3xl">🗝️</span> محرك إصدار التراخيص (License Generator)
                            </h3>
                            <p className="text-indigo-300 text-sm mb-8 relative z-10">قم بإنشاء كيان مؤسسي جديد وإصدار كود تفعيل مشفر له.</p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <button 
                                        ...
                                        className="w-full mt-4 bg-emerald-600 hover:bg-emerald-500 text-white font-black py-5 rounded-2xl shadow-xl shadow-emerald-600/20 transition-all flex justify-center items-center gap-2 active:scale-95"
                                    >
                                        <span className="text-xl">✨</span> توليد كود التفعيل وإنشاء المؤسسة
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-800 p-6 rounded-[2rem] border border-slate-700">
                            ...
                            <div className="space-y-3">
                                {licenses.map(lic => {
                                    const org = organizations.find(o => o.id === lic.org_id);
                                    return (
                                        <div key={lic.id} className="flex flex-wrap md:flex-nowrap justify-between items-center bg-slate-900 p-4 rounded-2xl border border-slate-700/50 gap-4">
                                            <div>
                                                <span className="block font-black text-indigo-400 text-lg font-mono tracking-wider">{lic.key}</span>

...
(function() {
    ...
    const App = () => {
        ...
        useEffect(() => {
            ...
            const markAppReady = () => {
                if (window.EcoFineSplash?.markAppReady) {
                    window.EcoFineSplash.markAppReady();
                } else {
                    // احتياطي في حال عدم توفر واجهة index.html الجديدة (نسخة قديمة من الصفحة مثلاً)
                    const splash = document.getElementById('splash-frame') || document.getElementById('splash-screen');
                    if (splash) {
                ...
                return (
                    <div className="animate-in fade-in duration-500 h-full">
                        <Component 
                            ...
                            initialView={activeTab === 'attendance' ? 'attendance' : 'list'}
                        />
                    </div>
                );
            }
            ...
            <div className="h-[100dvh] bg-slate-50 flex overflow-hidden flex-col print:h-auto print:bg-white" dir="rtl">
                ...
                <header className="print:hidden shrink-0 h-16 bg-white border-b border-slate-200 flex items-center px-4 z-40 shadow-sm">
                    ...
                    <div className="mr-auto flex items-center gap-3 md:gap-4">
                        <button
                            onClick={() => window.open('attendance.html', '_blank', 'noopener,noreferrer')}
                            className="hidden sm:flex items-center gap-2 bg-blue-600 text-white px-3 py-2 rounded-xl font-black text-[10px] shadow-sm hover:bg-blue-500 transition-colors active:scale-95"

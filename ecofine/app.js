/**
 * 🚀 app.js - المايسترو ومركز القيادة (The X-Command Center V12.1)
 * التحديث: تحسينات في الأداء، معالجة الأخطاء، دعم أفضل للموديولات، مع الحفاظ على الباب السري (0120).
 */
...
(function() {
    ...
    const App = () => {
        ...
        useEffect(() => {
            ...
            const initializeApp = async () => {
                try {
                    ...
                    alert(`❌ فشل تشغيل محرك إكس: ${err.message || 'خطأ غير معروف'}`);
                }
            };
        ...
        const handleSecretDoor = useCallback(() => {
            ...
            if (secretClicksRef.current >= SECRET_CLICKS_THRESHOLD) {
                ...
                secretClicksRef.current = 0; // تصفير بعد الفتح
            } else {
            ...
            <div className="h-[100dvh] bg-slate-50 flex overflow-hidden flex-col print:h-auto print:bg-white" dir="rtl">
                    ...
                    <div className="fixed inset-0 bg-slate-900/90 z-[9999] flex items-center justify-center backdrop-blur-md">
                        <div className="bg-slate-950 p-8 rounded-[2rem] border border-red-500/30 shadow-2xl text-center max-w-sm w-full animate-in zoom-in duration-300">
                            ...
                            <div className="flex gap-3">
                                <button 
                                    onClick={verifyPin} 
                                    className="flex-1 bg-red-600 text-white font-black py-3 rounded-xl hover:bg-red-500 transition-colors shadow-lg shadow-red-600/20"
                                >
                                    دخول
                                </button>
                                <button 
                                    onClick={() => { setShowPinModal(false); setPinCode(''); }} 
                                    className="flex-1 bg-slate-800 text-slate-400 font-black py-3 rounded-xl hover:bg-slate-700 transition-colors"
                                >
                                    إغلاق
                                </button>
                            </div>
                ...
                <header className="print:hidden shrink-0 h-16 bg-white/90 backdrop-blur-md border-b border-slate-200 flex items-center px-4 z-40 shadow-sm">
                    <button 
                        ...
                        className="p-2 text-slate-700 text-2xl active:scale-95 transition-transform md:hidden"
                    ...
                    <div 
                        className="mr-3 flex flex-col hidden sm:flex cursor-pointer select-none" 
                        onClick={handleSecretDoor}
                        role="button"
                        tabIndex={0}
                        aria-label="الضغط 5 مرات للدخول السيادي"
                    >
                ...
                <div className="flex flex-1 overflow-hidden relative">
                    ...
                    <aside className={`print:hidden absolute md:static inset-y-0 right-0 z-[100] w-72 bg-slate-900 text-slate-400 flex flex-col transition-transform duration-300 ease-in-out shadow-2xl ${isMenuOpen ? 'translate-x-0' : 'translate-x-full md:translate-x-0'}`}>
                        ...
                        <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-950 shrink-0">
                            ...
                            >
                                ...
                                <p className="text-[9px] text-blue-500 font-bold tracking-widest uppercase mt-1">Version 12.1</p>
                            </div>
                            <button 
                                onClick={() => setIsMenuOpen(false)} 
                                className="text-xl text-slate-500 hover:text-white transition-colors md:hidden"
                    ...
                    {/* طبقة التعتيم عند فتح القائمة في الموبايل */}
                    {isMenuOpen && (
                        <div 
                            className="fixed inset-0 bg-slate-900/60 z-[90] backdrop-blur-sm transition-opacity md:hidden" 
                            onClick={() => setIsMenuOpen(false)}
                            aria-hidden="true"
                        ></div>
                    )}
    ...
    /**
     * لوحة التحكم الديناميكية القابلة للتخصيص
     */
    const DashboardView = ({ currentUser, setActiveTab, rawMenuGroups }) => {
        ...
        const [dashConfig, setDashConfig] = useState(() => {
            try {
                ...
                console.warn("⚠️ فشل قراءة التخصيص:", e);
        ...
        useEffect(() => {
            ...
            const fetchStats = async () => {
                ...
                try {
                    const [invoices, installments, customers, products, legal, expenses] = await Promise.all([
                        ...
                        window.db.getAll('expenses').catch(() => [])
                    ]);
            ...
            fetchStats();
            intervalId = setInterval(fetchStats, DASHBOARD_REFRESH_INTERVAL);
        ...
        const saveConfig = useCallback(() => {
            try {
                ...
                console.warn("⚠️ فشل حفظ التخصيص:", e);
            }
            setIsEditing(false);
        }, [dashConfig, currentUser?.username]);

        const toggleStat = useCallback((statKey) => {
            setDashConfig(prev => {
        ...
        const toggleAction = useCallback((moduleId) => {
            setDashConfig(prev => {
            ...
            <div className="space-y-6 animate-in fade-in duration-700 pb-10 relative">
                ...
                <div className="flex justify-end mb-2">
                    {isEditing ? (
                        <button 
                            onClick={saveConfig} 
                            className="bg-green-600 text-white px-6 py-2 rounded-xl text-xs font-black shadow-md flex items-center gap-2 animate-pulse"
                        >
                            💾 حفظ شكل اللوحة
                        </button>
                    ) : (
                        <button 
                            ...
                            className="bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 px-4 py-2 rounded-xl text-[10px] font-black shadow-sm flex items-center gap-2 transition-colors"
                        >
                            ⚙️ تخصيص اللوحة
                        </button>
                    )}
                        ...
                        {dashConfig.actions.map(actionId => {
                            ...
                            return (
                                <button 
                                    ...
                                    className="flex-1 xl:flex-none flex items-center justify-center gap-2 bg-slate-50 border border-slate-200 px-4 py-3 md:py-4 rounded-2xl text-[10px] md:text-xs font-black text-slate-700 hover:bg-slate-100 active:scale-95 transition-all shadow-sm"
                                >
                ...
                {stats.lowStock > 0 && (
                    <div className="bg-orange-50 p-5 md:p-6 rounded-[2rem] border border-orange-200 flex items-start sm:items-center gap-4 shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] md:text-xs font-bold text-orange-600 leading-relaxed">
                                يوجد <span className="text-orange-800 font-black px-1">{stats.lowStock}</span> أصناف أوشكت على النفاذ من المخزن الرئيسي.
                            </p>
                        </div>
                        <button 
                            onClick={() => setActiveTab('inventory')} 
                            className="hidden sm:block mr-auto bg-orange-600 text-white px-4 py-2 rounded-xl text-[10px] font-black hover:bg-orange-700 transition-colors shadow-md"
                        >
                            عرض النواقص
                        </button>
                    </div>

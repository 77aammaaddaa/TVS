...
// ==========================================
// 5. محرك الطباعة الحرارية (Thermal Print Engine)
// ==========================================
    ...
    const htmlContent = `
        ...
        <head>
            ...
            <style>
                ...
                .header-title { font-size: 18px; font-weight: 900; margin-bottom: 5px; border-bottom: 2px dashed #000; padding-bottom: 5px; }
                ...
                th { font-size: 11px; }
                .total-section { font-size: 14px; font-weight: 900; margin-top: 10px; border-top: 2px solid #000; border-bottom: 2px solid #000; padding: 5px 0; }
        ...
        <body onload="window.print(); window.close();">
            ...
            <div class="info-row"><span>نوع البيع:</span> <span>${typeLabel}</span></div>
            ...
            ${invoice.shipping_fee > 0 ? `
                <div class="info-row"><span>رسوم الشحن:</span> <span>${invoice.shipping_fee} ${currency}</span></div>
            ` : ''}
            ...
            ${saleType === 'installment' && xCoreData ? `
                <div class="xcore-section">
                    ...
                    <div class="info-row"><span>المقدم المدفوع الآن:</span> <span>${xCoreData.downPayment.toLocaleString()} ${currency}</span></div>
                    <div class="info-row"><span>الباقي تقسيط على:</span> <span>${xCoreData.calculatedMonths} شهور</span></div>
                </div>
            ` : ''}

            <div class="footer">
                <div>شكراً لتسوقكم معنا!</div>
                <div>Powered by Eco Fine Pro V12.0</div>
            </div>
        </body>
        </html>
    `;
...
const processSale = async () => {
    ...
    try {
        ...
        // 🖨️ استدعاء محرك الطباعة قبل تفريغ الشاشة
...
return (
    <div className="h-full flex flex-col relative pb-32 animate-in fade-in">
        ...
        {/* 🚀 شاشة الدفع النهائية (Checkout Modal) */}
        {isCheckoutOpen && (
            <div className="fixed inset-0 z-[9999] bg-slate-100 w-full h-[100dvh] flex flex-col animate-in slide-in-from-bottom-full duration-300">
                ...
                {/* الفوتر الثابت السُفلي (زر الإتمام وطباعة الفاتورة) */}
                <div className="shrink-0 bg-white border-t border-slate-200 p-5 md:p-6 shadow-[0_-20px_40px_rgba(0,0,0,0.08)] z-50">
                    ...
                    >
                        ...
                        {isProcessingSale ? 'جاري تشفير وتسجيل الفاتورة...' : 'تأكيد العملية وطباعة الإيصال'}
                    </button>
                </div>
